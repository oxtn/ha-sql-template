...
async def async_setup_platform(
    ...
    query_template: Template = conf[CONF_QUERY]
...
class SQLSensor(SensorEntity):
    ...
    def update(self) -> None:
        ...
        try:
            query_text: str = self._query.async_render(parse_result=False)
            _LOGGER.debug("Query text: %s", query_text)
            result: Result = sess.execute(sqlalchemy.text(str(query_text)))
        except SQLAlchemyError as err:
            _LOGGER.error(
